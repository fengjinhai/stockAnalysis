...
def dealGetPrice(id, changeDate, stockCode, price, stockIndex, afterDay):
    ...
    if retVal:
        ...
        sql = 'update director_stock_change set %s=%f, %s=%f where id=%d'%(param1, stockChangePercent, param2, indexChangePercent, id)
        doDB(sql)
    ...
    if not retVal:
        ...
        if not retVal:
            ...
            if not retVal:
                ...
                retVal = getPrice(str(needDate), stockCode)
    if retVal:
        sql = 'update director_stock_change set dealTimeIndex = %f where id = %d'%(float(retVal[1]), int(id))
        doDB(sql)
